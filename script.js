/* ============================================================
   S&H Rentals Co — Event Equipment, Planning & Transport
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
const STORAGE_KEY = 'sh_rentals_data_v2';
const SESSION_KEY = 'sh_rentals_session';

const DEFAULTS = {
  business: {
    name: 'S&H Rentals Co',
    tagline: 'Event Equipment · Planning · Transport',
    phone: '+27614169953',
    ...
    about: 'S&H Rentals Co is a proudly Sowetan event services business. We supply event equipment rentals, professional event planning and reliable transportation for weddings, parties, corporate functions, funerals, church gatherings and community events across Soweto and greater Johannesburg. From a single tent to a full wedding for 500 guests, we handle the equipment, the décor, the setup and the shuttle logistics.',
    facebook: '',
    instagram: '',
    tiktok: ''
  },
  services: [
    {
      id: 'svc_1',
      title: 'Wedding Tent & Marquee Package',
      desc: 'Fully erected marquee tent, draped ceiling, flooring, side walls and lighting. Sized for 100–300 guests. Includes delivery, setup and collection.',
      price: '4500',
      unit: 'per event',
      badge: 'Weddings',
    ...
    {
      id: 'svc_2',
      title: 'Chairs, Tables & Linen Combo',
      desc: 'Stretch chairs, round or rectangular tables, tablecloths, runners and napkins. Priced per 50-guest set. Perfect for parties and church events.',
      price: '1800',
      unit: 'per event',
      badge: 'Most Booked',
      image: 'https://images.unsplash.com/photo-1478146896981-b80fe463b330?auto=format&fit=crop&w=900&q=80'
    },
    {
      id: 'svc_3',
      title: 'Sound, Stage & Lighting',
      desc: 'Professional PA system, wireless microphones, stage decking, LED par lights and up-lighting. Includes a sound technician for the day.',
      price: '3200',
      unit: 'per event',
      badge: 'Pro AV',
    ...
    {
      id: 'svc_4',
      title: 'Full Event Planning & Décor',
      desc: 'End-to-end planning: concept, décor styling, flower arrangements, centrepieces, draping, balloon arches and on-site coordination on the day.',
      price: '6500',
      unit: 'per event',
      badge: 'Full Service',
    ...
    {
      id: 'svc_5',
      title: 'Guest Shuttle & Minibus Hire',
      desc: 'Air-conditioned minibuses and shuttle vans with professional drivers to move your guests safely. Priced per trip within Soweto and Johannesburg.',
      price: '1500',
      unit: 'per trip',
      badge: 'Transport',
    ...
    {
      id: 'svc_6',
      title: 'Luxury Bridal Car Hire',
      desc: 'Executive sedan or SUV for the bridal couple, chauffeur-driven, decorated with ribbons and flowers. Includes 4-hour hire window.',
      price: '2200',
      unit: 'per event',
      badge: 'Luxury',
    ...
    {
      id: 'svc_7',
      title: 'Catering Equipment & Mobile Bar',
      desc: 'Chafing dishes, gas burners, serving utensils, crockery, cutlery, mobile bar counter and drinks coolers. Fully sanitised and ready to use.',
      price: '1400',
      unit: 'per event',
      badge: '',
    ...
    },
    {
      ...
      desc: 'Silent diesel generator with cabling and distribution box. Keeps your sound, lights and catering running even when the power goes out.',
      price: '1900',
      unit: 'per event',
      badge: 'Reliable',
...
function applyBranding() {
  ...
  ['topbarPhoneText', 'contactPhoneText', 'footerPhoneText'].forEach(function (id) {
    var el = document.getElementById(id);
    if (el) el.textContent = biz.phone;
  });

  var waText = document.getElementById('contactWaText');
  if (waText) waText.textContent = biz.phone;

  ['topbarPhone', 'heroCall', 'contactPhone', 'footerPhone', 'formFootPhone'].forEach(function (id) {
    var el = document.getElementById(id);
    if (el) el.setAttribute('href', telLink(biz.phone));
  });

  ['navWa', 'heroWa', 'aboutWa', 'customWa', 'contactWa', 'waFab', 'footerWa'].forEach(function (id) {
    var el = document.getElementById(id);
    if (el) {
      el.setAttribute('href', waLink(biz.whatsapp, 'Hi ' + biz.name + ', I would like to enquire about event equipment, planning and transport.'));
...
function getDefaultHeroFromDOM() {
  return 'https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&w=1800&q=80';
}
...
function renderServices() {
  ...
  state.services.forEach(function (svc) {
    ...
    var waMsg = 'Hi ' + state.business.name + ', I am interested in the "' + svc.title + '" package at ' + formatPrice(svc.price) + ' ' + svc.unit + '.';
...
function renderGallery() {
  ...
  state.gallery.forEach(function (src, i) {
    ...
    img.src = src;
    img.alt = state.business.name + ' event gallery image ' + (i + 1);
    img.loading = 'lazy';
    img.onerror = function () {
      this.onerror = null;
      this.src = placeholder('Event Gallery');
    };
  ...
  state.gallery.forEach(function (src, i) {
    ...
    item.innerHTML =
      '<img src="' + escapeAttr(src) + '" alt="Gallery ' + (i + 1) + '" ' +
        'onerror="this.onerror=null;this.src=\'' + placeholder('Event') + '\'">' +
...
function initServiceForm() {
  ...
  setupDropzone('svcDrop', 'svcFile', 'svcPreview', function (dataUrl) { editingImageData = dataUrl; });
}
...
function initSettings() {
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', function () {
      if (!confirm('This will erase ALL data and restore defaults. Continue?')) return;
